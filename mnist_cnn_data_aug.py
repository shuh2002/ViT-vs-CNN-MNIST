...
norm_resize_transformer = transforms.Compose([
    ...
    transforms.Lambda(lambda x : x + 2*torch.randn_like(x))
])
...
split_index = len(training_data) // 2
indices1 = list(range(split_index))
indices2 = list(range(split_index, len(training_data)))

dataset1 = [(training_data[i][0], training_data[i][1]) for i in indices1]
dataset2 = [(training_data[i][0], training_data[i][1]) for i in indices2]

dataset1_transformed = [(transforms.RandomVerticalFlip(0.6)(image), label) for image, label in dataset1]
dataset2_transformed = [(transforms.RandomHorizontalFlip(0.6)(image), label) for image, label in dataset2]

training_data = ConcatDataset([dataset1_transformed, dataset2_transformed])

test_data_01 = datasets.MNIST(
    root="data",
    train=False,
    download=True,
    transform=transforms.Compose([norm_resize_transformer, transforms.Lambda(lambda x : x + 0.1*torch.randn_like(x))])
)

test_data_001 = datasets.MNIST(
    root="data",
    train=False,
    download=True,
    transform=transforms.Compose([norm_resize_transformer, transforms.Lambda(lambda x : x + 0.01*torch.randn_like(x))])
)
test_data_1 = datasets.MNIST(
    root="data",
    train=False,
    download=True,
    transform=transforms.Compose([norm_resize_transformer, transforms.Lambda(lambda x : x + 1*torch.randn_like(x))])
)
...
test_loader_01 = DataLoader(test_data_01, batch_size=batch_size_test)
test_loader_001 = DataLoader(test_data_001, batch_size=batch_size_test)
test_loader_1 = DataLoader(test_data_1, batch_size=batch_size_test)
...
def train_test():
    ...
    for epoch in range(epochs):
        ...
        total_correct_001 = 0
        with torch.no_grad():
            # find test accuracy for different test sets
            ...
            for i, data in enumerate(test_loader_1):
                ...
                outputs = net(inputs)
                _, preds = torch.max(outputs.data, 1)
                total_correct_1 += (preds == labels).float().sum()
            list_1[epoch] = total_correct_1 / ((i + 1) * batch_size_test)
            for i, data in enumerate(test_loader_01):
                inputs, labels = data
                outputs = net(inputs)
                _, preds = torch.max(outputs.data, 1)
                total_correct_01 += (preds == labels).float().sum()
            list_01[epoch] = total_correct_01 / ((i + 1) * batch_size_test)
            for i, data in enumerate(test_loader_001):
                inputs, labels = data
                outputs = net(inputs)
                _, preds = torch.max(outputs.data, 1)
                total_correct_001 += (preds == labels).float().sum()
            list_001[epoch] = total_correct_001 / ((i + 1) * batch_size_test)
    ...
    return hflip_v_acc_list, vflip_v_acc_list, hflip_dict, vflip_dict, list_1, list_01, list_001


# train, test, graph results
hflip_acc, vflip_acc, hflip_dict, vflip_dict, list_1, list_01, list_001 = train_test()
x = np.arange(1, epochs+1)
plt.plot(x, hflip_acc, color='red', label='h-flip')
plt.plot(x, vflip_acc, color='blue', label='v-flip')
plt.legend()
plt.title('Test Accuracy (Flipped Images w/Data Aug.) vs Epoch')
plt.ylim([0, 1])
plt.xticks(x)
plt.savefig("flip_results_data_aug.png")
plt.clf()
plt.plot(x, list_1, color='red', label='var=1')
plt.plot(x, list_01, color='blue', label='var=0.1')
plt.plot(x, list_001, color='green', label='var=0.01')
plt.legend()
plt.title('Test Accuracy (Added Noise w/Data Aug.) vs Epoch')
plt.ylim([0, 1])
plt.xticks(x)
plt.savefig("noise_results_data_aug.png")
